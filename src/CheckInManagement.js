...
import { Box, Button, Typography, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, CircularProgress, Chip } from '@mui/material';
import { useParams } from 'react-router-dom';
import { db, auth } from './firebase'; // Import การเชื่อมต่อกับ Firestore
import { collection, getDocs, doc, getDoc, updateDoc, Timestamp, setDoc, serverTimestamp } from 'firebase/firestore';
...
if (!data.date) {
  ...
  updateDoc(checkinRef, { date: nowThaiFormat }).catch(err => {
    console.error("Error updating date:", err);
        ...
        if (data.date) {
          ...
          checkInStartTime = new Date(gregorianYear, month - 1, day, hours, minutes, seconds);
        } else {
          checkInStartTime = new Date();
        }
  
        // กำหนดเวลาที่ถือว่ามาสาย (เริ่มต้น + 15 นาที)
        const lateThreshold = new Date(checkInStartTime.getTime() + 15 * 60 * 1000);
        ...
        const allStudents = classroomStudents.map(student => {
          ...
          if (score && score.date && score.date !== 'ยังไม่เช็คชื่อ') {
            ...
            const [hours, minutes, seconds] = timePart.split(':');
            const checkInTime = new Date(gregorianYear, month - 1, day, hours, minutes, seconds);
  ...
  const isLateCheckIn = (checkInTimeStr, lateThresholdDate) => {
    try {
      ...
      const [hours, minutes, seconds] = timePart.split(':');
      const checkInDate = new Date(gregorianYear, month - 1, day, hours, minutes, seconds);
...
const handleCheckin = async () => {
  ...
  try {
    ...
    const scoreRef = doc(db, `classroom/${cid}/checkin/${cno}/scores`, currentUser.uid);
...
console.log('calculateLateTime:', checkinData && checkinData.date ? calculateLateTime(checkinData.date) : 'ไม่มีข้อมูล');
...
const updateStudentStatus = async (studentId, newStatus) => {
  ...
  const calculateLateTime = (dateString) => {
    try {
      ...
      // สร้าง Date object
      const originalDate = new Date(gregorianYear, month - 1, day, hours, minutes, seconds);
      ...
      const formattedDate = 
        `${String(lateTime.getDate()).padStart(2, '0')}/${String(lateTime.getMonth() + 1).padStart(2, '0')}/${buddhistLateYear} ` +
        `${String(lateTime.getHours()).padStart(2, '0')}:${String(lateTime.getMinutes()).padStart(2, '0')}:${String(lateTime.getSeconds()).padStart(2, '0')}`;
...
const loadClassroomStudents = async () => {
  ...
  if (!checkinData) {
    return (
      <Box sx={{ p: 3 }}>
        <Typography variant="h5" gutterBottom>ไม่พบข้อมูลการเช็คชื่อ</Typography>
      </Box>
    );
  }
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Typography variant="h4" gutterBottom>การเช็คชื่อครั้งที่ {cno}</Typography>
      
      <Box sx={{ mb: 3 }}>
    <Typography variant="h6">
        สร้างเมื่อ: {checkinData && checkinData.date ? 
        checkinData.date : 'ไม่มีข้อมูล'}
    </Typography>
    <Typography variant="body1">
        เวลาที่หมดเขต (จะถือว่ามาสาย): {checkinData && checkinData.date ?
        calculateLateTime(checkinData.date) : 'ไม่มีข้อมูล'}
    </Typography>
    </Box>
      ...
      {currentUser && !isTeacher && (
        <Box sx={{ mb: 3 }}>
          ...
          >
            {students.some(s => s.id === currentUser.uid) ? 'คุณได้เช็คชื่อไปแล้ว' : 'เช็คชื่อเข้าเรียน'}
          </Button>
        </Box>
      )}
