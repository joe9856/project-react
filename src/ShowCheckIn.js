...
const ShowCheckIn = () => {
  ...
  const fetchCheckIns = async () => {
    ...
    const checkInList = checkInSnap.docs.map((doc) => ({
      id: doc.id,
      ...doc.data(),
    }));
    ...
    const countPromises = checkInList.map(async (checkIn) => {
      const studentRef = collection(
        db,
        `classroom/${cid}/checkin/${checkIn.id}/students`
      );
  ...
  const endCheckIn = async (checkInId, cid) => {
    if (!checkInId || !cid) {
      console.error("Error: checkInId หรือ cid เป็นค่าว่าง", {
        checkInId,
        cid,
      });
    ...
    try {
      const studentsRef = collection(
        db,
        `classroom/${cid}/checkin/${checkInId}/students`
      );
      ...
      for (const student of studentsList) {
        const studentScoreRef = doc(
          db,
          `classroom/${cid}/checkin/${checkInId}/scores/${student.stdid}`
        );
        ...
        if (
          student.timestamp &&
          typeof student.timestamp.toDate === "function"
        ) {
  ...
  const viewAnswers = (checkInId) => {
    ...
    navigate(`/questionAnswers/${cid}/checkin/${checkInId}/answers`);
  };

  return (
    <Box className="container mt-5">
      <Typography variant="h4" className="text-center mb-4">
        รายการการเช็คชื่อสำหรับห้องเรียน{" "}
        {classroomName ? classroomName : "Loading..."}
      </Typography>

      <TableContainer
        component={Paper}
        sx={{ mt: 2, p: 2, borderRadius: 2, boxShadow: 3 }}
      >
        <Table>
          <TableHead>
            ...
            {checkIns.map((checkIn) => (
              <TableRow key={checkIn.id} hover>
                ...
                <TableCell>
                  ...
                  {checkIn.status === 1 && (
                    <Chip label="กำลังเช็คชื่อ" color="warning" />
                  )}
                  {checkIn.status === 2 && (
                    <Chip label="เสร็จแล้ว" color="success" />
                  )}
                </TableCell>
                <TableCell>
                  {checkIn.status === 2 && (
                    <Typography variant="body1" fontWeight="bold">
                      {checkInCounts[checkIn.id]} คน
                    </Typography>
                  )}
                </TableCell>
                <TableCell>
                ...
                <TableCell>
                  <Button
                    ...
                    onClick={() =>
                      navigate(
                        `/showcheckinstudents/${cid}/checkin/${checkIn.id}`
                      )
                    }
                  >
                    ดูรายชื่อ
                ...
                <TableCell>
                  <Button
                    ...
                    onClick={() =>
                      navigate(
                        `/checkinmanagement/${cid}/checkin/${checkIn.id}`
                      )
                    }
                  >
                    ดูคะแนน
